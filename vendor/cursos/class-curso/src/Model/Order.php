 ...
 class Order extends Model{
    ...
    public function save(){
       ...
       $results = $sql->select("CALL sp_order_save(:idcontrol, :desname, :desstate, :descity, :nrphone, :desshift, :desconsultant,
         :desteacher, :deshelp, :desprice, :course_con, :desdate, :payment)", [
...
public static function getP($s){
   ...
   return $sql->select("SELECT COUNT(MONTH(dtregister))  AS mest from tb_control WHERE desteacher = :s AND MONTH(dtregister) LIKE MONTH(CURRENT_DATE())",[
       ":s"=>$s
   ]);
